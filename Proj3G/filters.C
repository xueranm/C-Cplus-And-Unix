...
Shrinker::Execute(){
	...
	int half_w = (int)img1->get_w()/2;
	int half_h = (int)img1->get_h()/2;
    ...
    for (i=0; i< half_w;i++){
    	for(j =0;j<half_h;j++){
    		output->getData()[3*(j*half_w+i)] = img1->getData()[3*(4*j*half_w + 2*i)];
    		output->getData()[3*(j*half_w+i)+1] = img1->getData()[3*(4*j*half_w + 2*i)+1];
    		output->getData()[3*(j*half_w+i)+2] = img1->getData()[3*(4*j*half_w + 2*i)+2];
    	}
    }
};
